...
Status: Stages 0–2b land in one PR on `main` (this branch). Stage 0 findings
are in `docs/stage0-prerender-findings.md` (measurement only — no spike product
code). Image bytes remain Stage 3.
...
Stages below describe the build order; they ship together here, then Stage 3+
continues.
...
**Done — 2026-08-01.** Measured on the Stage 0 spike; tables in
`docs/stage0-prerender-findings.md` (findings land in this PR; spike product
code does not). Result and corrected reading in §6.1: no route became ƒ, so
nothing is blocked. Content routes stay client-only for cost reasons, and the
acceptance bar above should be read as "no route becomes ƒ," not "no route
becomes ◐."
...
**Done in this PR.** Follow-ups included restoring `/sign-in/:path+` → `/sign-in`
and moving synthetic emails to RFC 2606 `.invalid` (§5.2, §13.1).
...
**Done in this PR.** Authz-first tests (cross-user + 401), `FOR UPDATE` then
`max(seq)+1` in the same txn, CSRF via `screenCsrf` (live cross-site → 403),
dual `/api/responses` contract, disconnect / stream `cancel` → `incomplete`
via `after()`, local adoption with same UUID PKs (IndexedDB cleared only after
success), soft delete + export, `/cleo` ◐ with content routes still ○. Authz /
persist suites need a live `DATABASE_URL` and fail loud in CI when absent.
...
temporarily worse. Stage 3 closes this; do not leave a long gap before Stage 3
if image chats matter.
