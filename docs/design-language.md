...
Results arrive grouped — Topics, Countries, Space, Civilizations, Cities, Oceans, Rivers,
...
`--page-gap` (2.75rem / 3.25rem from 40rem), `--page-gap-cluster`
(1.5rem / 1.75rem), and `--page-gap-head` (0.75rem). Apply them through
...
decision — and do not leave `.ama-*` styles in `app/globals.css` for a surface
that no longer exists. OpenAI is the only **model** third-party API; account
auth is self-hosted Better Auth on Neon (see [`auth.md`](./auth.md)).
...
[`oceans.md`](./oceans.md), [`rivers.md`](./rivers.md). Visual
contract below.
