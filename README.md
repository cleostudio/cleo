...
Oceans field guides at `/oceans`; Biomes field guides at `/biomes`; Elements
field guides at `/elements`; a Sky atlas at `/sky`; Compare at `/compare`; a
place Gallery at `/gallery`; a Topics catalog; Writing (for a future encyclopedia
layer); and a browser-only agent with streamed Markdown, vision, image
...
- Elements guides: `lib/elements.ts` + `content/element-photos.json` + JPEGs in
...
- Cleo agent: `components/cleo/*`, `lib/cleo/*` (instructions include the
  Explore/Space/Oceans/Biomes/Elements catalog so replies can deep-link field
  guides and embed curated topic photographs when a visual helps)
