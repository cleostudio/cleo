...
describe('public post metadata', () => {
  it('fits localized titles and descriptions within social preview budgets', () => {
    for (const post of getAllPosts()) {
      expect(`${post.title} | Cleo`.length, post.slug).toBeLessThanOrEqual(70)
      expect(`${post.titleEn} | Cleo`.length, post.slug).toBeLessThanOrEqual(70)
      expect(post.description?.length ?? 0, post.slug).toBeGreaterThan(0)
      expect(post.description?.length ?? 0, post.slug).toBeLessThanOrEqual(160)
