...
describe('portal catalog instructions', () => {
  it('lists Explore, Space, Civilizations, Cities, Oceans, and Rivers guides with exact site paths', () => {
    ...
    expect(block).toContain(`Rivers guides (${riverSubjects.length}):`)
    ...
    expect(block).toContain('Nile (/rivers/nile)')
    ...
    expect(block).toContain('[Rivers](/rivers)')
  ...
  it('is appended to the shipped Cleo instructions', () => {
    ...
    expect(CLEO_INSTRUCTIONS).toContain('(/rivers/nile)')
  })
})
