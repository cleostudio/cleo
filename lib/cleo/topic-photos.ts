/**
 * Resolve curated Explore/Space/Civilizations/Cities/Oceans/Rivers photographs for a Cleo
...
import { citySubjects, getCitySubject } from '~/lib/cities'
import {
...
import { getRiverSubject, riverSubjects } from '~/lib/rivers'
...
export type TopicPhotoCollection =
  | 'explore'
  | 'space'
  | 'civilizations'
  | 'cities'
...
function loadTopicPhotos(
  ...
  if (topic.collection === 'cities') {
    const subject = getCitySubject(topic.slug)
    if (!subject) return []
    return subject.photos.map((photo, index) => ({
      collection: 'cities' as const,
      slug: subject.slug,
  ...
  if (topic.collection === 'rivers') {
    const subject = getRiverSubject(topic.slug)
    if (!subject) return []
    return subject.photos.map((photo, index) => ({
      collection: 'rivers' as const,
      slug: subject.slug,
...
export function matchTopicPhotosInText(text: string): TopicPhoto[] {
  ...
  const pathPattern =
    /(?:^|[^A-Za-z0-9])\/(explore|space|civilizations|cities|oceans|rivers)\/([a-z0-9-]+)(?![a-z0-9-])/gi
...
The following complete curated photograph sets are from this website's Explore, Space, Civilizations, and Cities topics. When the user's question is about these subjects:
- You MAY and SHOULD include a curated photograph in your reply when appearance, landscape, what something looks like, or a sense of place would help — or when the user asks to see a photo/image.
