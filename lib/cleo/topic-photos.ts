...
import { getElementSubject, elementSubjects } from '~/lib/elements'
...
export type TopicPhoto = {
  collection: 'explore' | 'space' | 'oceans' | 'biomes' | 'elements'
  slug: string
  name: string
  href: string
...
function loadTopicPhoto(
  ...
  if (topic.collection === 'elements') {
    const subject = getElementSubject(topic.slug)
    if (!subject) return null
    return {
      collection: 'elements',
      slug: subject.slug,
...
export function matchTopicPhotosInText(text: string): TopicPhoto[] {
  ...
  const pathPattern =
    /(?:^|[^A-Za-z0-9])\/(explore|space|oceans|biomes|elements)\/([a-z0-9-]+)(?![a-z0-9-])/gi
  for (const match of haystack.matchAll(pathPattern)) {
    const collection = match[1] as
      | 'explore'
      | 'space'
      | 'oceans'
      | 'biomes'
...
The following curated photographs are from this website's Explore, Space, Oceans, Biomes, and Elements topics. When the user's question is about these subjects:
...
- Still weave one Markdown deep link to the field guide (\`[Name](/explore/…)\`, \`[Name](/space/…)\`, \`[Name](/oceans/…)\`, \`[Name](/biomes/…)\`, or \`[Name](/elements/…)\`) on first mention.
- Prefer these curated photos over \`image_generation\` for real places, space bodies, ocean waters, biomes, and element specimens. Use \`image_generation\` only if the user asks you to create, draw, redesign, or invent a visual the curated photo cannot cover (diagram, stylized illustration, edit).
