...
const MARKDOWN_GUIDE_LINK =
  /\[([^\]]*)\]\((\/(explore|space|oceans|biomes|elements)\/([a-z0-9-]+))\)/gi

/** Curated static JPEGs under the site image roots. */
const CURATED_TOPIC_IMAGE_SRC =
  /^\/images\/(atlas|space|oceans|biomes|elements)\/[a-z0-9-]+\/w(640|1280|2048)\.jpg$/
...
export function cleanPortalGuideLabel(
  ...
  const cleaned = trimmed
    .replace(
      /\s*(?:explore|space|oceans?|biomes?|elements?)?\s*(?:field\s*)?guides?\s*$/i,
      '',
    )
    .replace(/^(?:explore|space|oceans|biomes|elements)\s*[·|:–-]\s*/i, '')
    .replace(/^(?:the\s+)?(?:explore|space|oceans|biomes|elements)\s+/i, '')
    .trim()
...
function stripLeadingGuideChrome(block: string) {
  ...
  while (remainder !== previous) {
    ...
    remainder = remainder
      ...
      .replace(/^(?:explore|space|oceans|biomes|elements)\s+/i, '')
      .trim()
  }
...
export const CLEO_PORTAL_STARTERS = [
  ...
  {
    label: 'What is tundra?',
    prompt:
      'What defines tundra as a biome? Deep-link the Biomes field guide when you name it.',
  },
  {
    label: 'Orient me to iron',
    prompt:
      'Give me a quick orientation to iron as an element. Deep-link its Elements field guide when you mention it.',
  },
] as const
