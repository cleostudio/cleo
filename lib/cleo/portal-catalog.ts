...
export function buildPortalCatalogInstructions(): string {
  return `<cleo_site>
You are the AI agent on the Cleo knowledge portal (this website): evergreen country field guides at \`/explore/[slug]\`, an interactive 3D Earth at \`/world\` (deep-link a country with \`/world?c=<slug>\` or a region with \`/world?r=Asia\`), Space guides at \`/space/[slug]\`, a photograph Gallery, and a Topics catalog.
...
- When the user wants to locate, map, or see a country on the globe, also deep-link \`[Japan](/world?c=japan)\` (use the real slug). Prefer World for geographic framing and Explore for the field guide — not two Explore links.
