...
import { riverSubjectsByCategory } from '~/lib/rivers'
import { publicPageMetadata } from '~/lib/public-page-metadata'

export function riversPageMetadata() {
  const copy = publicPageMetadata.rivers
...
export function RiversPageView() {
  const categories = riverSubjectsByCategory()

  return (
    <div className="mx-auto w-full max-w-content">
      <div className="flex items-start justify-between gap-4">
        <header className="max-w-content-narrow">
          <h1 className="page-eyebrow enter">Rivers</h1>
          <p
            className="page-introduction enter mt-4 text-balance"
            style={{ '--enter-delay': '70ms' } as React.CSSProperties}
          >
            {publicPageMetadata.rivers.description}
          </p>
        </header>
      ...
      <div className="mt-10 flex flex-col gap-10">
        {categories.map(([category, subjects]) => {
          ...
          return (
            <section key={category} aria-labelledby={`rivers-${category}`}>
              <h2
                id={`rivers-${category}`}
