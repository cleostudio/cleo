...
export function citySubjectStaticParams() {
  return citySubjectSlugs().map((slug) => ({ slug }))
}

export function citySubjectMetadata(slug: string) {
  const subject = getCitySubject(slug)
  if (!subject) return {}

  return localeMetadata({
    path: `/cities/${subject.slug}`,
    title: subject.name,
    description: cityDescription(subject).slice(0, 160),
  })
}

export function CitySubjectPageView({ slug }: { slug: string }) {
  const subject = getCitySubject(slug)
  if (!subject) notFound()
  ...
  return (
    <article className="field-guide mx-auto w-full max-w-content">
      <div className="flex items-start justify-between gap-4">
        ...
        <PixelCluster variant={8} className="enter shrink-0" />
      </div>
      ...
      <section
        ...
        aria-labelledby="city-about"
      >
        <h2 id="city-about" className="guide-label">
          About
        </h2>
      ...
      <section
        ...
        aria-labelledby="city-features"
      >
        <h2 id="city-features" className="guide-label">
          Features
        </h2>
      ...
      <section
        ...
        aria-labelledby="city-facts"
      >
        <h2 id="city-facts" className="guide-label">
          Fact plate
        </h2>
        <dl className="spec-plate spec-plate-guide mt-3">
          ...
          <div>
            <dt>Founded</dt>
            <dd>{subject.facts.founded}</dd>
          </div>
          <div>
            <dt>Capital role</dt>
            <dd>{subject.facts.capitalRole}</dd>
          </div>
          <div>
            <dt>Corridors</dt>
            <dd>{subject.facts.corridors}</dd>
          </div>
          <div>
            <dt>Explore</dt>
            <dd>
      ...
      <section
        ...
        aria-labelledby="city-sources"
      >
        <h2 id="city-sources" className="guide-label">
          Sources
        </h2>
