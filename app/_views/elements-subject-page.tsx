...
export function elementSubjectStaticParams() {
  return elementSubjectSlugs().map((slug) => ({ slug }))
}

export function elementSubjectMetadata(slug: string) {
  const subject = getElementSubject(slug)
  if (!subject) return {}

  return localeMetadata({
    path: `/elements/${subject.slug}`,
    title: subject.name,
    description: elementDescription(subject).slice(0, 160),
  })
}

export function ElementsSubjectPageView({ slug }: { slug: string }) {
  const subject = getElementSubject(slug)
  if (!subject) notFound()
  ...
  return (
    <article className="field-guide mx-auto w-full max-w-content px-6">
      <div className="flex items-start justify-between gap-4">
        <header className="max-w-content-narrow">
          <p className="page-eyebrow enter">
            <Link href="/elements" className="hover:text-foreground">
              Elements
            </Link>
      ...
      <section
        ...
        aria-labelledby="elements-about"
      >
        <h2 id="elements-about" className="guide-label">
          Orientation
        </h2>
      ...
      <section
        ...
        aria-labelledby="elements-features"
      >
        <h2 id="elements-features" className="guide-label">
          Features
        </h2>
      ...
      <section
        ...
        aria-labelledby="elements-facts"
      >
        <h2 id="elements-facts" className="guide-label">
          Fact plate
        </h2>
        <dl className="spec-plate spec-plate-guide mt-3">
          <div>
            ...
            <dd>
              ...
              {subject.facts.atomicNumber}
            </dd>
          </div>
          <div>
            <dt>Symbol</dt>
            <dd>{subject.facts.symbol}</dd>
          </div>
          <div>
            <dt>Period</dt>
            <dd>{subject.facts.period}</dd>
          </div>
          <div>
            <dt>Group</dt>
            <dd>{subject.facts.group}</dd>
          </div>
          <div>
            <dt>Standard state</dt>
            <dd>{subject.facts.standardState}</dd>
          </div>
          <div>
            <dt>Catalog</dt>
            <dd>{subject.code}</dd>
      ...
      <section
        ...
        aria-labelledby="elements-sources"
      >
        <h2 id="elements-sources" className="guide-label">
          Sources
        </h2>
