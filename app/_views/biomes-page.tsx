...
import { biomeSubjectsByCategory } from '~/lib/biomes'
import { publicPageMetadata } from '~/lib/public-page-metadata'

export function biomesPageMetadata() {
  const copy = publicPageMetadata.biomes
...
export function BiomesPageView() {
  const categories = biomeSubjectsByCategory()
    ...
    <div className="mx-auto w-full max-w-content px-6">
      <div className="flex items-start justify-between gap-4">
        <header className="max-w-content-narrow">
          ...
          >
            {publicPageMetadata.biomes.description}
          </p>
        </header>
      ...
      <div className="mt-10 flex flex-col gap-10">
        {categories.map(([category, subjects]) => {
          ...
          return (
            <section key={category} aria-labelledby={`biomes-${category}`}>
              <h2
                id={`biomes-${category}`}
