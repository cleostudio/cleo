...
import { civilizationSubjectsByCategory } from '~/lib/civilizations'
import { localeMetadata } from '~/lib/locale-metadata'
import { publicPageMetadata } from '~/lib/public-page-metadata'

export function civilizationsPageMetadata() {
  const copy = publicPageMetadata.civilizations
...
export function CivilizationsPageView() {
  const categories = civilizationSubjectsByCategory()

  return (
    <div className="mx-auto w-full max-w-content">
      <div className="flex items-start justify-between gap-4">
        <header className="max-w-content-narrow">
          <h1 className="page-eyebrow enter">Civilizations</h1>
          <p
            className="page-introduction enter mt-4 text-balance"
            style={{ '--enter-delay': '70ms' } as React.CSSProperties}
          >
            {publicPageMetadata.civilizations.description}
          </p>
        </header>
      ...
      <div className="mt-10 flex flex-col gap-10">
        {categories.map(([category, subjects]) => {
          ...
          return (
            <section key={category} aria-labelledby={`civilizations-${category}`}>
              <h2
                id={`civilizations-${category}`}
