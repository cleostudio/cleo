...
- Country orientation prose is curated (`scripts/atlas/atlas-about.json`), never
  ...
  page.
- Place/space/civilization images are static JPEGs under
  `public/images/{atlas,space,civilizations}/` with browser `srcset`. No image
  CDN, account, or `/_next/image` re-encode at runtime.
...
- Render model output through Streamdown, never raw HTML. Invented
  Explore/Space/Civilizations paths are stripped by `lib/cleo/guardrails.ts`.
