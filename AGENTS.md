...
- Generative interactive widgets: the model may emit fenced `cleo` JSON
  (`tabs`, `timeline`, `facts`, `compare`, `steps`, `cards`) parsed by
  ...
  steps, focusable compare subjects, optional guide hrefs) — not quizzes or
  suggestion chips. Incomplete fences are dropped while streaming so raw JSON
  never flashes.
