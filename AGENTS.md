...
- Generative interactive widgets: the model may emit fenced `cleo` JSON
  (`tabs`, `timeline`, `facts`, `compare`, `steps`, `cards`, `gallery`) parsed
  by `lib/cleo/interactive.ts` and rendered by `components/cleo/interactive.tsx`
  inside `components/cleo/markdown.tsx`. These are in-answer widgets the user
  manipulates in place (tabs, expandable facts/timeline/cards, progressive
  steps, focusable compare subjects, curated photo galleries with ZoomImage,
  optional guide hrefs) — not quizzes or suggestion chips. Incomplete fences
