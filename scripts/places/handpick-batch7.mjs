...
const UA = 'cleo-places-curate/1.0 (batch7-handpick-persist)'

const JOBS = [
  ['houston', ['Houston Texas skyline day', 'Downtown Houston skyline']],
  ['porto', ['Porto Ribeira Douro Portugal', 'Dom Luis Bridge Porto']],
  ['naxos', ['Naxos Portara', 'Naxos Chora harbour Greece']],
  ['masai-mara', ['Maasai Mara landscape plain', 'Masai Mara savanna Kenya']],
  [
    'sagrada-familia',
    ['Sagrada Familia Barcelona towers', 'Sagrada Família exterior'],
  ],
  ['lyon', ['Lyon Fourviere Saone', 'Lyon confluence skyline France']],
  ['seville', ['Plaza de Espana Seville', 'Seville Cathedral Giralda']],
  ['cologne', ['Cologne Cathedral Rhine', 'Kölner Dom exterior']],
  ['gobi', ['Gobi Desert dunes Mongolia', 'Khongoryn Els Gobi']],
  ['guadeloupe', ['Guadeloupe beach Caribbean', 'La Soufriere Guadeloupe']],
  ['accra', ['Accra Ghana Independence Arch', 'Accra coastline Ghana']],
  ['menorca', ['Menorca cala beach', 'Ciutadella Menorca harbour']],
]
...
async function fileInfo(title) {
  ...
  return {
    title: page.title,
    url: info.url,
    sourceUrl: info.sourceUrl || info.descriptionurl,
    width: info.width,
    height: info.height,
...
async function searchPick(query) {
  ...
  for (const page of Object.values(data.query?.pages ?? {})) {
    ...
    if (
      /\b(map|flag|cat|lion|animal|bird|church --|coptic|funchal|madeira|baths of|statue|fontaine|house 1 to|creole|food|meal|portrait|szambala)\b/i.test(
        title,
      )
    ) {
      continue
...
console.log('All batch7 handpicks saved')
