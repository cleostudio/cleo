...
const UA = 'cleo-places-curate/1.0 (batch6-handpick-persist)'

const JOBS = [
  ['miami', ['Miami skyline Biscayne Bay', 'Downtown Miami skyline day']],
  ['geneva', ['Jet d Eau Geneva', 'Geneva waterfront Jet dEau', 'Geneva lake city skyline']],
  ['casablanca', ['Hassan II Mosque Casablanca', 'Casablanca Hassan II ocean']],
  ['wellington', ['Wellington harbour skyline', 'Wellington New Zealand waterfront']],
  ['pantanal', ['Pantanal wetland landscape Brazil', 'Pantanal floodplain aerial']],
  ['brittany', ['Brittany coast France cliffs', 'Saint Malo Brittany walls', 'Cote de Granit Rose']],
  ['oahu', ['Diamond Head Waikiki', 'Honolulu Diamond Head panorama', 'Waikiki Beach Diamond Head']],
  ['busan', ['Busan Haeundae skyline', 'Busan harbour bridge skyline']],
  ['krakow', ['Krakow Main Square Cloth Hall', 'Wawel Castle Krakow']],
  ['mount-cook', ['Aoraki Mount Cook summit', 'Mount Cook New Zealand peak']],
  ['antelope-canyon', ['Upper Antelope Canyon light beam', 'Antelope Canyon sandstone interior']],
  ['burj-khalifa', ['Burj Khalifa Dubai daytime', 'Burj Khalifa from Dubai Fountain']],
]
...
async function searchPick(query) {
  ...
  for (const page of Object.values(data.query?.pages ?? {})) {
    ...
    if (
      /\b(map|flag|statue|living|jabiru|bird|animal|train|oncf|reservoir|kaitoke|sundial|chillon|cathedral|beomeosa|temple gate|sefton|hippo)\b/i.test(
        title,
      )
    ) {
      continue
...
console.log('All batch6 handpicks saved')
