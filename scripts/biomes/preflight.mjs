#!/usr/bin/env node
/**
 * Validate checked-in Biomes photo metadata against public JPEG renditions.
 */
...
const photos = JSON.parse(
  readFileSync(join(root, 'content/biome-photos.json'), 'utf8'),
)
const sources = JSON.parse(
  readFileSync(join(root, 'scripts/biomes/biome-photo-sources.json'), 'utf8'),
)
...
for (const slug of sourceSlugs) {
  if (!photos[slug]) errors.push(`${slug}: missing from content/biome-photos.json`)
}
...
for (const [slug, photo] of Object.entries(photos)) {
  ...
  for (const rendition of photo.renditions) {
    if (!rendition.src?.startsWith(`/images/biomes/${slug}/`)) {
...
if (errors.length) {
  console.error('Biomes photo preflight failed:\n' + errors.join('\n'))
  process.exit(1)
}

console.log(
  `Biomes photo preflight ok (${photoSlugs.length} subjects, ${photoSlugs.length * 3} JPEGs)`,
)
