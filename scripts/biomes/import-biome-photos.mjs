#!/usr/bin/env node
/**
 * NASA / agency imagery → optimized local Biomes renditions.
 *
 ...
 *    public/images/biomes/{slug}/
 * 3. Write credits + checksum + rendition metadata into content/biome-photos.json
 *
 * The app serves those static files directly — never via a CDN account or
 * /_next/image. Originals stay in .biome-originals/ (gitignored).
 */
...
const ORIGINALS = join(root, '.biome-originals')
const PUBLIC_BIOMES = join(root, 'public/images/biomes')
const sourcesPath = join(root, 'scripts/biomes/biome-photo-sources.json')
const outputPath = join(root, 'content/biome-photos.json')
...
mkdirSync(PUBLIC_BIOMES, { recursive: true })
...
async function downloadOriginal(slug, url, dest) {
  ...
  const res = await fetch(url, {
    headers: { 'user-agent': 'cleo-biome-import/1.0', accept: 'image/*' },
...
async function buildRenditions(slug, originalBuffer) {
  const dir = join(PUBLIC_BIOMES, slug)
  ...
  for (const targetWidth of WIDTHS) {
    ...
    renditions.push({
      width: targetWidth,
      src: `/images/biomes/${slug}/w${targetWidth}.jpg`,
...
for (const slug of slugs) {
  ...
  if (!source) {
    errors.push(`${slug}: missing from biome-photo-sources.json`)
    continue
  }
  if (
    !force &&
...
console.log(`\nWrote content/biome-photos.json (${Object.keys(photos).length} subjects)`)
console.log(`Renditions under public/images/biomes/`)
console.log(`Originals under .biome-originals/ (keep gitignored)`)
