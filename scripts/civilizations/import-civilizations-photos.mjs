#!/usr/bin/env node
/**
 * Wikimedia Commons image sets → optimized local Civilizations renditions.
 *
 ...
 *    photographs under public/images/civilizations/{slug}/
 * 3. Write credits + checksum + rendition metadata into
 *    content/civilizations-photos.json
 *
 * The app serves those static files directly — never via a CDN account or
 * /_next/image. Originals stay in .civilizations-originals/ (gitignored).
 */
...
const ORIGINALS = join(root, '.civilizations-originals')
const PUBLIC_DIR = join(root, 'public/images/civilizations')
const sourcesPath = join(
  root,
  'scripts/civilizations/civilizations-photo-sources.json',
)
const outputPath = join(root, 'content/civilizations-photos.json')
  ...
  for (let attempt = 1; attempt <= 6; attempt++) {
    try {
      const res = await fetch(url, {
        headers: {
          'user-agent':
            'cleo-civilizations-import/1.0 (https://github.com/cleostudio/cleo; knowledge-portal photo import)',
          accept: 'image/*',
        },
...
async function buildRenditions(slug, photoIndex, originalBuffer) {
  ...
  for (const targetWidth of WIDTHS) {
    ...
    renditions.push({
      ...
      src: `/images/civilizations/${slug}/${filename}`,
...
console.log(
  `\nWrote content/civilizations-photos.json (${Object.keys(photoSets).length} subjects × 3 photographs)`,
)
console.log(`Renditions under public/images/civilizations/`)
console.log(`Originals under .civilizations-originals/ (keep gitignored)`)
