#!/usr/bin/env node
/**
 * Validate checked-in Civilizations photo metadata against public JPEG renditions.
 */
...
const photos = JSON.parse(
  readFileSync(join(root, 'content/civilizations-photos.json'), 'utf8'),
)
const sources = JSON.parse(
  readFileSync(
    join(root, 'scripts/civilizations/civilizations-photo-sources.json'),
    'utf8',
  ),
)
...
for (const [slug, photoSet] of Object.entries(photos)) {
  ...
  for (const [photoIndex, photo] of photoSet.entries()) {
    const ctx = `civilizations/${slug} photo ${photoIndex + 1}`
    ...
    if (sourceSet?.[photoIndex]?.commonsTitle !== photo.commonsTitle) {
      errors.push(`${ctx}: commonsTitle does not match curated source`)
    }
    if (
    ...
    const renditionPath = new RegExp(
      `^/images/civilizations/${slug}/w(?:640|1280|2048)${slot}\\.jpg$`,
    )
    ...
    if (sourceUrls.has(photo.sourceUrl)) {
      errors.push(`civilizations/${slug}: duplicate photograph source`)
    }
    if (checksums.has(photo.checksum)) {
      errors.push(`civilizations/${slug}: duplicate photograph content`)
    }
...
if (errors.length) {
  console.error('Civilizations photo preflight failed:\n' + errors.join('\n'))
  process.exit(1)
}
...
const renditionCount = Object.values(photos).reduce(
  (total, photoSet) =>
    ...
    (Array.isArray(photoSet)
      ? photoSet.reduce(
          (count, photo) => count + (photo.renditions?.length ?? 0),
          0,
        )
      : 0),
  0,
...
console.log(
  `Civilizations photo preflight ok (${photoSlugs.length} subjects × 3 photographs, ${renditionCount} JPEGs)`,
)
