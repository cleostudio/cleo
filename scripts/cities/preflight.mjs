#!/usr/bin/env node
/**
 * Validate checked-in Cities photo metadata against public JPEG renditions.
 */
...
const photos = JSON.parse(
  readFileSync(join(root, 'content/cities-photos.json'), 'utf8'),
)
const sources = JSON.parse(
  readFileSync(
    join(root, 'scripts/cities/cities-photo-sources.json'),
    'utf8',
  ),
)
...
for (const [slug, photoSet] of Object.entries(photos)) {
  ...
  for (const [photoIndex, photo] of photoSet.entries()) {
    const ctx = `cities/${slug} photo ${photoIndex + 1}`
    ...
    const renditionPath = new RegExp(
      `^/images/cities/${slug}/w(?:640|1280|2048)${slot}\\.jpg$`,
    )
    ...
    if (sourceUrls.has(photo.sourceUrl)) {
      errors.push(`cities/${slug}: duplicate photograph source`)
    }
    if (checksums.has(photo.checksum)) {
      errors.push(`cities/${slug}: duplicate photograph content`)
    }
...
if (errors.length) {
  console.error('Cities photo preflight failed:\n' + errors.join('\n'))
  process.exit(1)
}
...
console.log(
  `Cities photo preflight ok (${photoSlugs.length} subjects × 3 photographs, ${renditionCount} JPEGs)`,
)
