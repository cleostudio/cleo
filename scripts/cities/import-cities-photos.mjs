#!/usr/bin/env node
/**
 * Wikimedia Commons image sets → optimized local Cities renditions.
 *
 ...
 *    photographs under public/images/cities/{slug}/
 * 3. Write credits + checksum + rendition metadata into
 *    content/cities-photos.json
 *
 * The app serves those static files directly — never via a CDN account or
 * /_next/image. Originals stay in .cities-originals/ (gitignored).
 */
...
const ORIGINALS = join(root, '.cities-originals')
const PUBLIC_DIR = join(root, 'public/images/cities')
const sourcesPath = join(
  root,
  'scripts/cities/cities-photo-sources.json',
)
const outputPath = join(root, 'content/cities-photos.json')
  ...
  for (let attempt = 1; attempt <= 6; attempt++) {
    try {
      const res = await fetch(url, {
        headers: {
          'user-agent':
            'cleo-cities-import/1.0 (https://github.com/cleostudio/cleo; knowledge-portal photo import)',
          accept: 'image/*',
        },
...
async function buildRenditions(slug, photoIndex, originalBuffer) {
  ...
  for (const targetWidth of WIDTHS) {
    ...
    renditions.push({
      ...
      src: `/images/cities/${slug}/${filename}`,
...
console.log(
  `\nWrote content/cities-photos.json (${Object.keys(photoSets).length} subjects × 3 photographs)`,
)
console.log(`Renditions under public/images/cities/`)
console.log(`Originals under .cities-originals/ (keep gitignored)`)
