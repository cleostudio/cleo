#!/usr/bin/env node
/**
 * Wikimedia Commons image sets → optimized local Rivers renditions.
 *
 ...
 *    photographs under public/images/rivers/{slug}/
 * 3. Write credits + checksum + rendition metadata into
 *    content/rivers-photos.json
 *
 * The app serves those static files directly — never via a CDN account or
 * /_next/image. Originals stay in .rivers-originals/ (gitignored).
 */
...
const ORIGINALS = join(root, '.rivers-originals')
const PUBLIC_DIR = join(root, 'public/images/rivers')
const sourcesPath = join(
  root,
  'scripts/rivers/rivers-photo-sources.json',
)
const outputPath = join(root, 'content/rivers-photos.json')
  ...
  for (let attempt = 1; attempt <= 6; attempt++) {
    try {
      const res = await fetch(url, {
        headers: {
          'user-agent':
            'cleo-rivers-import/1.0 (https://github.com/cleostudio/cleo; knowledge-portal photo import)',
          accept: 'image/*',
        },
...
async function buildRenditions(slug, photoIndex, originalBuffer) {
  ...
  for (const targetWidth of WIDTHS) {
    ...
    renditions.push({
      ...
      src: `/images/rivers/${slug}/${filename}`,
...
console.log(
  `\nWrote content/rivers-photos.json (${Object.keys(photoSets).length} subjects × 3 photographs)`,
)
console.log(`Renditions under public/images/rivers/`)
console.log(`Originals under .rivers-originals/ (keep gitignored)`)
