...
const photos = JSON.parse(
  readFileSync(join(root, 'content/oceans-photos.json'), 'utf8'),
)
const sources = JSON.parse(
  readFileSync(
    join(root, 'scripts/oceans/oceans-photo-sources.json'),
    'utf8',
  ),
)
...
for (const [slug, photoSet] of Object.entries(photos)) {
  ...
  for (const [photoIndex, photo] of photoSet.entries()) {
    const ctx = `oceans/${slug} photo ${photoIndex + 1}`
    ...
    const renditionPath = new RegExp(
      `^/images/oceans/${slug}/w(?:640|1280|2048)${slot}\\.jpg$`,
    )
    ...
    if (sourceUrls.has(photo.sourceUrl)) {
      errors.push(`oceans/${slug}: duplicate photograph source`)
    }
    if (checksums.has(photo.checksum)) {
      errors.push(`oceans/${slug}: duplicate photograph content`)
    }
...
console.log(
  `Oceans photo preflight ok (${photoSlugs.length} subjects × 3 photographs, ${renditionCount} JPEGs)`,
)
