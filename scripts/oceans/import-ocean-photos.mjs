#!/usr/bin/env node
/**
 * NASA / agency imagery → optimized local Oceans renditions.
 *
 ...
 * 3. Write credits + checksum + rendition metadata into content/ocean-photos.json
 *
 * The app serves those static files directly — never via a CDN account or
 * /_next/image. Originals stay in .ocean-originals/ (gitignored).
 */
...
const ORIGINALS = join(root, '.ocean-originals')
const PUBLIC_OCEANS = join(root, 'public/images/oceans')
const sourcesPath = join(root, 'scripts/oceans/ocean-photo-sources.json')
const outputPath = join(root, 'content/ocean-photos.json')
...
mkdirSync(PUBLIC_OCEANS, { recursive: true })
...
async function downloadOriginal(slug, url, dest) {
  ...
  const res = await fetch(url, {
    headers: { 'user-agent': 'cleo-ocean-import/1.0', accept: 'image/*' },
...
async function buildRenditions(slug, originalBuffer) {
  const dir = join(PUBLIC_OCEANS, slug)
  ...
  for (const targetWidth of WIDTHS) {
    ...
    renditions.push({
      width: targetWidth,
      src: `/images/oceans/${slug}/w${targetWidth}.jpg`,
...
for (const slug of slugs) {
  ...
  if (!source) {
    errors.push(`${slug}: missing from ocean-photo-sources.json`)
    continue
  }
  if (
    !force &&
...
console.log(`\nWrote content/ocean-photos.json (${Object.keys(photos).length} subjects)`)
console.log(`Renditions under public/images/oceans/`)
console.log(`Originals under .ocean-originals/ (keep gitignored)`)
