...
describe('DockFallback', () => {
  it('keeps the dock shell useful while route state resolves', () => {
    ...
    expect(screen.getByRole('link', { name: /Trail/ }).getAttribute('href')).toBe(
      '/trail',
    )
    ...
    expect(hrefs.indexOf('/explore')).toBeLessThan(hrefs.indexOf('/trail'))
    expect(hrefs.indexOf('/trail')).toBeLessThan(hrefs.indexOf('/topics'))
